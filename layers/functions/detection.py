...
class Detect(object):
    ...
    def __call__(self, predictions, net):
        ...
        with timer.env('Detect'):
            ...
            for batch_idx in range(batch_size):
                ...
                result = self.opd_detect(batch_idx, conf_preds, decoded_boxes, axes, mask_data, inst_data)
    ...
    def detect(self, batch_idx, conf_preds, decoded_boxes, mask_data, inst_data):
        ...
        # boxes, masks, classes, scores = self.opd_fast_nms(boxes, masks, scores, self.nms_thresh, self.top_k)
    ...
    def opd_detect(self, batch_idx, conf_preds, decoded_boxes, axes, mask_data, inst_data):
        """ Perform nms for only the max scoring class that isn't background (class 0) """
        cur_scores = conf_preds[batch_idx, 1:, :]
        conf_scores, _ = torch.max(cur_scores, dim=0)
        ...
        boxes, axes, masks, classes, scores = self.opd_fast_nms(boxes, axes, masks, scores, self.nms_thresh, self.top_k)

        return {'box': boxes, 'axes':axes, 'mask': masks, 'class': classes, 'score': scores}
    ...
    def opd_fast_nms(self, boxes, axes, masks, scores, iou_threshold:float=0.5, top_k:int=200, second_threshold:bool=False):
        ...
        axes =  axes[idx.view(-1), :].view(num_classes, num_dets, 3)
        ...
        return boxes, axes, masks, classes, scores
