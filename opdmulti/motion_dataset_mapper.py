...
class MyMotionDatasetMapper(DatasetMapper):
    @configurable
    def __init__(
        self,
        use_gt=False,
    ...
    def __call__(self, dataset_dict):
        ...
        # aug_input = T.AugInput(image, sem_seg=None) 
        # image = aug_input.image
        # image = cv2.imread("/data92/lisq2309/OPDMulti-Release/mydataset/MotionDataset/valid/93-3660.png", cv2.IMREAD_UNCHANGED)
        image = cv2.imread("/data92/lisq2309/OPDMulti-Release/mydataset/MotionDataset/valid/"+file_name, cv2.IMREAD_UNCHANGED)
        dataset_dict["image"] = torch.as_tensor(
            np.ascontiguousarray(image.transpose(2, 0, 1))
        )
